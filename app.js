...
function refreshUrl() {
  return WORKER_URL.replace(/\/news(?:\?.*)?$/, '/refresh');
}
...
async function triggerBackgroundRefresh(showMessage = false) {
  try {
    const res = await fetch(refreshUrl() + '?bust=' + Date.now(), {
      cache: 'no-store',
      headers: { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' },
    });

    if (showMessage && res.ok) showToast('最新ニュースの収集を開始しました', 'info');
  } catch (e) {
    console.warn('[SIGNAL] refresh trigger failed:', e);
  }
}
...
async function startAutoRefreshCheck() {
  ...
  const check = async () => {
    ...
    try {
      const res = await fetch(WORKER_URL + '?meta=1&bust=' + Date.now(), {
        cache: 'no-store',
        headers: { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' },
      });
    ...
    if (checks < maxChecks) setTimeout(check, 20 * 1000);
  };

  setTimeout(check, 20 * 1000);

  setInterval(async () => {
    await triggerBackgroundRefresh(false);
    checks = 0;
    setTimeout(check, 20 * 1000);
  }, 15 * 60 * 1000);
}
...
async function load(options = {}) {
  ...
  try {
    const res = await fetch(WORKER_URL + '?bust=' + Date.now(), {
      cache: 'no-store',
      headers: { 'Cache-Control': 'no-cache', 'Pragma': 'no-cache' },
    });
    ...
    cats.forEach(cat => {
      newsData[cat] = Array.isArray(raw[cat])
        ? raw[cat].map(a => ({
            ...a,
            title: clean(a.title || ''),
...
function renderList() {
  ...
  let arts = activeCategory === 'all'
    ? (searchQuery ? allArticles.filter(matchSearch) : allArticles)
    : (newsData[activeCategory] || []);

  if (activeCategory !== 'all' && searchQuery) arts = arts.filter(matchSearch);
  ...
  if (!arts.length) {
    feed.innerHTML = `<div class="empty-state"><h3>記事が見つかりません</h3>
      <p style="font-size:12px;margin-top:6px;color:var(--ink3)">媒体フィルタを確認してください</p></div>`;
    return;
  }
  ...
  document.getElementById('sourceCategories').innerHTML = `
    <div class="source-group">
      ...
      <div class="refresh-actions">
        <button class="refresh-action-btn primary" onclick="closeSettings();triggerBackgroundRefresh(true);">
          <span class="icon">↻</span>
          <div>
            <div class="btn-title">最新ニュースの収集を開始</div>
            <div class="btn-sub">画面はそのまま使えます。収集完了後に自動反映します</div>
          </div>
        </button>
      </div>
    </div>
...
window.triggerBackgroundRefresh = triggerBackgroundRefresh;
...
.refresh-action-btn .btn-sub{font-size:11px;color:rgba(247,245,240,.6);font-family:var(--ff-mono)}
