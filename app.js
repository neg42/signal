...
document.getElementById(‘feed’).innerHTML=` <div class="empty-state"> <h3>データを取得できません</h3> <p style="font-size:12px;margin-top:8px;color:var(--ink3)">Actions タブから手動実行してください</p> <p style="font-family:var(--ff-mono);font-size:10px;margin-top:8px;color:var(--ink3)">${esc(e.message)}</p> <button onclick="load()" style="margin-top:16px;padding:9px 20px;background:var(--ink);color:var(--paper);border:none;border-radius:4px;cursor:pointer">再読み込み</button> </div>`;
}
}
...
if(!arts.length){
feed.innerHTML=`<div class="empty-state"><h3>記事が見つかりません</h3> <p style="font-size:12px;margin-top:6px;color:var(--ink3)">Actionsを手動実行すると更新されます</p></div>`;
return;
}
...
<div class="std-desc">${a.description ? esc(a.description) : ‘📰 ’ + esc(a.source) + ’ の記事’}</div>
<div class="std-foot">
...
document.getElementById(‘articleBody’).innerHTML=` <div class="item-cat" style="color:${color}"><span class="dash"></span>${esc(cat?.label||a.category)}</div> <h1>${esc(a.title)}</h1> <div class="art-meta">${esc(a.source)}<span class="dot">·</span>${new Date(a.date).toLocaleString('ja-JP')}</div> <p class="art-body">${a.description ? esc(a.description) : ''}</p> <a class="art-link" href="${esc(a.url)}" target="_blank" rel="noopener">元記事を読む →</a>`;
...
document.getElementById(‘sourceCategories’).innerHTML=` <div class="source-group"> <div class="source-group-title">収集の仕組み</div> <div style="padding:12px;background:#fff;border:1px solid var(--paper3);border-radius:4px;font-size:12.5px;line-height:1.8;color:var(--ink2)"> GitHub Actionsが15分ごとに<strong>Google News RSS（日本語）</strong>を収集。 英語記事は自動除外されます。 </div> </div> <div class="source-group"> <div class="source-group-title">収集ソース</div> ${[ ['Google News RSS','社会・政治・経済・テクノロジー・エンタメ（日本語のみ）'], ['GIGAZINE','テクノロジー'], ['ファミ通 / 電撃 / IGN Japan','エンタメ・ゲーム'], ['ORICON NEWS / ナタリー','エンタメ'], ].map(([n,c])=>`<div class="source-item">
<span class="source-badge rss">RSS</span>
<label>${n}</label>
<small style="color:var(--ink3);font-size:10px;margin-left:auto">${c}</small>
</div>`).join('')} </div> <div class="source-group"> <div class="source-group-title">手動実行</div> <a href="https://github.com/neg42/signal/actions" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;gap:6px;background:var(--ink);color:var(--paper);padding:9px 16px;border-radius:4px;font-size:13px;text-decoration:none"> GitHub Actions を開く → </a> </div>`;
