...
document.getElementById(‘feed’).innerHTML=` <div class="empty-state"> <h3>データを取得できません</h3> <p style="font-size:12px;margin-top:8px;color:var(--ink3)">エラー内容: ${esc(e.message)}</p> <p style="font-family:var(--ff-mono);font-size:10px;margin-top:12px;color:var(--ink3);text-align:left;white-space:pre-wrap;max-width:90%;margin-left:auto;margin-right:auto">${esc((e.stack||'').slice(0,400))}</p> <button onclick="load()" style="margin-top:16px;padding:9px 20px;background:var(--ink);color:var(--paper);border:none;border-radius:4px;cursor:pointer">再読み込み</button> </div>`;
}
}
...
if(top3.length){
html+=`<div class="sec top-sec"> <div class="sec-hd"><span class="sec-lbl">最新ニュース</span> <button class="sec-more" onclick="setCategory('all','すべてのニュース')">すべて見る →</button> </div> ${top3.map((a,i)=>heroCard(a,i)).join('')} </div>`;
}
...
html+=`<div class="sec cat-sec"> <div class="sec-hd"> <span class="sec-lbl" style="color:${color}">${cat.icon} ${cat.label}</span> <button class="sec-more" onclick="setCategory('${cat.id}','${cat.label}')">もっと見る →</button> </div> ${arts.map(a=>compactCard(a,color)).join('')} </div>`;
});
...
return `<div class="hero-card${idx===0?' hero-first':''}" data-id="${esc(a.id)}"> <div class="item-cat" style="color:${color}"><span class="dash"></span>${esc(cat?.label||a.category)}</div> <div class="hero-ttl">${esc(a.title)}</div> ${a.description ? `<div class="hero-desc">${esc(a.description)}</div>` : ‘’}
...
return `<div class="compact-card" data-id="${esc(a.id)}"> <span class="bullet" style="background:${color}"></span> <div class="compact-inner"> <div class="compact-ttl">${esc(a.title)}</div> ${a.description ? `<div class="compact-desc">${esc(a.description)}</div>` : ‘’}
<div class="item-meta">${esc(a.source)}<span class="dot">·</span>${ago(a.date)}</div>
</div>
...
if(!arts.length){
feed.innerHTML=`<div class="empty-state"><h3>記事が見つかりません</h3> <p style="font-size:12px;margin-top:6px;color:var(--ink3)">媒体フィルタを確認するか、Actionsを手動実行してください</p></div>`;
return;
}
...
const cat=CATEGORIES.find(c=>c.id===a.category);
const color=CAT_COLORS[a.category]||’#555’;
return `<article class="std-card"> <div class="item-cat" style="color:${color}"><span class="dash"></span>${esc(cat?.label||a.category)}</div> <div class="std-ttl">${esc(a.title)}</div> ${a.description ? `<div class="std-desc">${esc(a.description)}</div>` : ‘’}
<div class="std-foot">
...
document.getElementById(‘articleBody’).innerHTML=`<div class="item-cat" style="color:${color}"><span class="dash"></span>${esc(cat?.label||a.category)}</div> <h1>${esc(a.title)}</h1> <div class="art-meta">${esc(a.source)}<span class="dot">·</span>${new Date(a.date).toLocaleString('ja-JP')}</div> ${a.description ?`<p class="art-body">${esc(a.description)}</p>` : '<p class="art-body" style="color:var(--ink3);font-style:italic">本文は元記事でご確認ください</p>'} <a class="art-link" href="${esc(a.url)}" target="_blank" rel="noopener">元記事を読む →</a>`;
...
li.innerHTML=`<a class="${activeCategory===cat.id?'active':''}" data-cat="${cat.id}"> <span class="cat-icon">${cat.icon}</span> <span>${cat.label}</span> ${count>0?`<span class="cat-count">${count}</span>`:''} </a>`;
...
band.innerHTML = `<div class="breaking-label">速報</div> <div class="breaking-list"> ${breakingNews.slice(0,5).map(b =>`
...
return `<div class="src-row ${blocked?'blocked':''}"> <input type="checkbox" id="src-${esc(name)}" data-src="${esc(name)}" ${blocked?'':'checked'}/> <label for="src-${esc(name)}">${esc(name)}</label> <span class="src-count">${count}</span> </div>`;
}).join(’’)
: ‘<p style="color:var(--ink3);font-size:12px;padding:8px 0">まだ媒体情報がありません</p>’;

document.getElementById(‘sourceCategories’).innerHTML = ` <div class="source-group"> <div class="source-group-title">最新ニュースを取得</div> <div class="refresh-actions"> <button class="refresh-action-btn primary" onclick="closeSettings();window._manualRefresh=true;load();"> <span class="icon">🔄</span> <div> <div class="btn-title">今すぐ最新ニュースを取得</div> <div class="btn-sub">Cloudflareサーバーから最新データを取得します</div> </div> </button> </div> <div style="padding:10px 12px;background:var(--paper2);border-radius:4px;font-size:11.5px;line-height:1.7;color:var(--ink3);margin-top:10px"> 💡 アクセスのたびに自動で最新ニュースを取得します。キャッシュは10分間保持されます。 </div> </div> <div class="source-group"> <div class="source-group-title">媒体フィルタ <small style="font-weight:400;color:var(--ink3);margin-left:8px">チェックを外すと非表示</small></div> <div style="display:flex;gap:6px;margin-bottom:10px"> <button class="src-mini-btn" onclick="selectAllSources()">すべて表示</button> <button class="src-mini-btn" onclick="clearAllSources()">すべて非表示</button> <button class="src-mini-btn" onclick="resetBlocks()">リセット</button> </div> <div class="src-list">${sourceList}</div> </div>`;
