 ...
 * 修正内容:
 * - App側のカテゴリ名に合わせて出力する
 *   domestic / world / business / entertainment / sports / it / science / local / life
 * - 国際ニュースを politics ではなく world に出力
 * - ITニュースを tech ではなく it に出力
 * - 国内ニュースを society ではなく domestic に出力
 * - entertainment はそのまま維持
 */
'use strict';
...
function parseJpRSS(xml, sourceName, category) {
  ...
  while ((m = itemRe.exec(xml)) !== null) {
    ...
    const get = tag => {
      ...
      const p = new RegExp(`<${tag}[^>]*>([\\s\\S]*?)<\\/${tag}>`, 'i').exec(chunk);
      return p ? p[1] : '';
    };
...
const GN_PARAMS = '?hl=ja&gl=JP&ceid=JP:ja';

function gnTopic(topic, cat) {
  return {
    url: `https://news.google.com/rss/headlines/section/topic/${topic}${GN_PARAMS}`,
    cat,
  };
}

const GN_SOURCES = [
  gnTopic('NATION', 'domestic'),
  { url: `https://news.google.com/rss${GN_PARAMS}`, cat: 'domestic' },
  gnTopic('WORLD', 'world'),
  gnTopic('BUSINESS', 'business'),
  gnTopic('ENTERTAINMENT', 'entertainment'),
  gnTopic('SPORTS', 'sports'),
  gnTopic('TECHNOLOGY', 'it'),
  gnTopic('SCIENCE', 'science'),
  gnTopic('HEALTH', 'life'),
];

const YT_BASE = 'https://www.youtube.com/feeds/videos.xml?channel_id=';
const YT_SOURCES = [
  { url: `${YT_BASE}UCGCZAYq5Xxojl_tSXcVJhiQ`, cat: 'domestic', name: 'ANN テレ朝（YouTube）' },
  { url: `${YT_BASE}UCoQBJMzcwmXrRSHBFAlTsIw`, cat: 'domestic', name: 'FNN プライムオンライン（YouTube）' },
  { url: `${YT_BASE}UC6AG81pAkf6Lbi_1VC5NmPA`, cat: 'domestic', name: 'TBS NEWS DIG（YouTube）' },
];

const JP_RSS = [
  { url: 'https://gigazine.net/news/rss_2.0/', cat: 'it', name: 'GIGAZINE' },
  ...
  { url: 'http://news.livedoor.com/topics/rss/top.xml', cat: 'domestic', name: 'ライブドアニュース' },
  { url: 'http://news.livedoor.com/topics/rss/dom.xml', cat: 'domestic', name: 'ライブドア国内' },
  { url: 'http://news.livedoor.com/topics/rss/int.xml', cat: 'world', name: 'ライブドア国際' },
  ...
  { url: 'http://news.livedoor.com/topics/rss/sci.xml', cat: 'science', name: 'ライブドア科学' },
];
...
async function enrichDescriptions(articles) {
  ...
  for (let i = 0; i < targets.length; i += DESC_CONCURRENCY) {
    ...
    if (i + DESC_CONCURRENCY < targets.length) {
...
async function main() {
  ...
  const preDist = {};
  all.forEach(a => {
    preDist[a.category || 'undefined'] = (preDist[a.category || 'undefined'] || 0) + 1;
  });
  ...
  if (fs.existsSync(newsPath)) {
    fs.copyFileSync(newsPath, path.join(OUT, 'news.backup.json'));
  }
  ...
  console.log('\n=== 媒体一覧（上位20）===');
  sortedSrc.slice(0, 20).forEach(([n, c]) => {
